angular.module('AppChat').controller('HashController', ['$http', '$log', '$scope', '$location', '$routeParams', function($http, $log, $scope, $location, $routeParams) {
        ...
        this.newText = "";
         ...
         this.loadMessages = function(){
            ...
            var url = "http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/messages/hashtagsSearch/" + chatId+"/"+word;
        ...
        this.postMsg = function(){
            ...
            var userId = "";
            $http.get("http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/id").then(function (response) {
                    userId = response.data.ID.id;
                    var url = "http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/chats/"+$routeParams.chat+"/messages/post/"+userId+"/"+message;
                    $http.post(url);
                    });
        ...
        this.likeMessage = function (like, messageId) {
            ...
            $http.get("http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/id").then(function (response) {
                    userId = response.data.ID.id;
                    var url = "http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/chats/"+userId+"/messages/"+messageId+"/like/"+like;
                    $http.post(url);
                    });
        }
