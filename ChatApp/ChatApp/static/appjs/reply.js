angular.module('AppChat').controller('ReplyController', ['$http', '$log', '$scope', '$location', '$routeParams', function($http, $log, $scope, $location, $routeParams) {
        ...
        this.replyMessage = function (like, messageId) {
            ...
            $http.get("http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/id").then(function (response) {
                    ...
                    var url = "http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/chats/"+$routeParams.cid+"/messages/post/"+userId+"/'RE: "+original+"' "+message;
                    $http.post(url).then(function(response){
                            console.log(response);
                            mid = response.data.MID.mid;
                            console.log('NEW MESSAGE: ',mid);
                            var rUrl = "http://chatapp.us-east-1.elasticbeanstalk.com/ChatApp/chats/messages/reply/"+mid+"/"+$routeParams.mid;
                            $http.post(rUrl);
                        });
                    });
